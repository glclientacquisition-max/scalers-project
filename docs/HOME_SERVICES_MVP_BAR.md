...
5. **Book:** service + name + when + landmark → `create_appointment`. Speak nothing until the backend. Progress: “Okay.” Ask the name once. After it is on file, name the job then collect when then landmark. Do not stall on name confirm.
...
Signup (DID + notify)
  ...
  → Owner visit notify
  → Inbox Visits (Confirm on List, Today, or Week)
```
