...
Owner prefs live on `tenants.notify_channels` (`{sms,whatsapp,email}`) and are edited in Business Settings → Assistant → **Notify channels**. Desk greys channels that are not platform-live yet (WhatsApp automated alerts = coming soon). Voice dispatch skips disabled prefs.
