...
9. Action turns (`CREATE_REQUEST` / `CAPTURE` / `ESCALATE` / `TRANSFER`) speak an immediate progress line before Gemini+tools; do not leave dead air. `TRANSFER` / `liveTransfer` is gated on `VOICE_LIVE_TRANSFER=on`. Executor is conference hold plus `POST /v1/calls` (`/voice/transfer-agent`). Do not close Stream for cold Dial. Close Stream only after outbound originate succeeds. Do not claim a bridge from the media loop. SautiKit outbound cost is KES 3/min answered; tenant outbound is KES 4/min (Ops owns rates).
