...
4. Lead capture: name + reason only when required for a saved request or justified handoff; confirm unclear names. STT variants auto-match to a returning-caller file name or a Kenya given-name table (`src/conversation/callerNameMatch.js`) so the spoken/saved spelling is canonical (Isha → Aisha). Do not invent names. Do not write free-form caller names into the TTS lexicon.
