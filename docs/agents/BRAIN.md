...
10. Call summary / primary intent persist instantly from Brain state + tools (STT text). Gemini does **not** hear live audio. After hangup, a fire-and-forget **transcript review** (`src/conversation/callTranscriptReview.js`, Flash-Lite, ~6s timeout) may rewrite `summary.reason` and only upgrade `needs_human` when merge rules agree. Kill switch: `POST_CALL_GEMINI_REVIEW=off`. Ignore STT fragments/backchannels as caller name or goal text; prefer `human` when handoff was requested.
