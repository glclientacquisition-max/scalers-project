...
| Filter tabs | `FilterTabs` + `filterTabClass`. Underline, `min-h-11`, active `border-[#0096FF] text-[#005CCC]`. Inbox List/Work and Today/Week share this. Contacts sort is `ContactSortSelect` (Last call · Name). Exclusive chips use `deskRateCardClass` (filled active `#005CCC`, muted idle, snap row): Inbox purpose (`InboxFilterPills`), Contacts segments All · Recents · Favourites · Saved · Unsaved (`contactFilterPills` on one `InboxFilterPills` row), and Settings 2 to 3 option enums (`SettingsSegmented`, including Update duration). Inbox purpose nouns stay Inbox-only. |
| Person-file KPI | `ContactKpiStrip` on `/contacts/[id]`. Flex row. Cards only when sourced. Interactions = ticket/call rows. Visits done = jobs `done`. Customer since = `N mo` from earliest stamp. Hide when unknown or 0 (`0 mo` does not paint). No wallpaper, Online, or mock counts. |
| List row | `DeskRowHit` in `deskRowHit.tsx`. Parent `relative`. Body `deskRowMutedClass`. Trailing verb `deskRowActionClass`. No Call, Open, or View column. Anatomy: `RowIdentity` circle (neutral, initials or person glyph, never tinted by state), identity plus **one** preview line (`deskPreviewClass` / `truncate`), timestamp right. Needs you rows may add one muted next-step line (`inboxNeedsYouNextStep`) under the preview. Same recipe as the Action dock. Unread (customer event after last open) is `RowStateDot` (brand blue, sits with the timestamp; `LivePing` when the row is live and unread) plus `deskRowWeightClass` (semibold while unread, medium once opened). Inbox pin is `InboxPinMark` (muted thumbtack next to time when `inbox_pinned_at` is set). Not a hit. Not Contacts Favourites. Needs you is a pile and nav badge, not the dot. A row that appears while watching uses `DeskLandSurface`. No opacity dimming. Full hangup copy is on the call, not in the list. |
...
| Pagination | `ui/Pagination.tsx` for URL lists (`?page=`, `min-h-11`). `ui/ListPager.tsx` for in-memory lists (catalog, pronunciation). `clampListPage` pulls a stale page back onto the last real page. History keeps View more. |
| Field | `deskFieldClass`. Settings: `settingsFieldClass` = `mt-1` + `deskFieldClass` |
| Sticky save | `settingsStickyHeaderClass` under `--desk-header-h`. Profile is a packed inner rail (`md:w-max md:max-w-[13.5rem]`) plus a fluid panel. Segmented strips are content-width, not a full-bleed justified bar. |
| Dialog | `DeskDialog`: overlay, Escape, focus restore. No enter animation. |
| Desk back | `DeskBack` plus `DeskRecordLead` in `DeskBack.tsx`. Icon-only chevron, 44px muted ghost, destination as `aria-label` / `DeskHint`. Nested insides (ticket, contact file, import, Archived, Profile panels) put the chevron in the first content row with the title or identity. Never a solo back row. Trail hits stay on the right. Ticket and Archived use `align="center"`. Contact file uses `start` so the chevron sits with the name. |
...
| Contact strip | `ContactStrip` on `/calls/[id]` header. Same family as the Contacts Phase 1 list row: `RowIdentity`, name or Unsaved / Name this caller, one `contactListSubline` (`deskPreviewClass`). Allowed facts: Unsaved, phone, last call. Identity only. Call + WhatsApp live on the ticket Action dock, not here. No `callId` write-back. No `lead_status` or Needs you invent. No Meta theater. Tap identity opens `/contacts/[id]`. Same row at every width. Never Online, last seen, presence, sent, or delivered. |
| Ticket Action dock | Full-width row under the split on `/calls/[id]`. Mark done, Call, WhatsApp, Ping teammate. The only Call and WhatsApp on the ticket. Same `deskHitClass` hits plus a visible label. Not in More. Confirm and hold Done stay on their banners. |
...
| Call | `CallLink` (`tel:` deep link, the device dialer places the call). List dock, ticket action dock, and contact header. Not the ticket strip. Rounded handset glyph (`data-icon="handset"`), brand `text-accent-deep`, muted tile `h-12 w-12` bordered with a light accent wash. Sits left of WhatsApp. `DeskHint` label Call. Never a filled primary, never a desk-telephone silhouette. |
