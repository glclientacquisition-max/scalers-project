...
| List row | `DeskRowHit` in `deskRowHit.tsx`. Parent `relative`. Body `deskRowMutedClass`. Trailing verb `deskRowActionClass`. No Call, Open, or View column. Anatomy: `RowIdentity` circle (neutral, initials or person glyph, never tinted by state), identity plus **one** preview line (`deskPreviewClass` / `truncate`), timestamp right. Needs you (`itemInNeedsYouPile`: `needsYou` and not archived, same set as the nav badge) is `RowStateDot` (brand blue, sits with the timestamp; `LivePing` when the row is live) plus `deskRowWeightClass` (semibold while Needs you, medium otherwise). Opening the ticket does not clear the dot. A row that appears while watching uses `DeskLandSurface`. No opacity dimming. Full hangup copy is on the call, not in the list. |
