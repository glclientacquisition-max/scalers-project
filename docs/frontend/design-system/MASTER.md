...
- Page title: nested records use `pageTitleClass` (`clamp(1.5rem, 2.4vw, 2rem)` semibold). List roots (Inbox, Contacts, Usage) use `deskListTitleClass` (`text-3xl sm:text-4xl` semibold, Sora via `font-display`). Not sticky. Overview uses the in-page Scalers lockup plus one muted workspace-and-date meta line, not a list-root H1. Profile keeps a compact `text-xl` hub title.
- Table: `text-sm`
