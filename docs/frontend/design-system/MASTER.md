...
| Preview truncate | `deskPreviewClass` (`min-w-0 truncate`) on list copy. Table cells that hold that copy use `deskPreviewCellClass` (`max-w-0`) so the ellipsis can fire. Never `overflow-wrap: anywhere` on a preview. Home queue units stay count nouns. A single short slot (`Tue 14:00`) may replace the unit. A hangup sentence may not. |
