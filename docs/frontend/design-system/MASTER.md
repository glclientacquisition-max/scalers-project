...
| Filter tabs | `FilterTabs` + `filterTabClass`. Underline, `min-h-11`, active `border-[#0096FF] text-[#005CCC]`. Inbox List/Work and Today/Week share this. Contacts sort is `ContactSortSelect` (Last call · Name). Exclusive chips use `deskRateCardClass` (filled active `#005CCC`, muted idle, snap row): Inbox purpose (`InboxFilterPills`), Contacts segments All · Saved · Unsaved, Contacts Recents · Favourites · Unsaved (`ContactPileStrip`), and Settings 2 to 3 option enums (`SettingsSegmented`, including Update duration). Inbox purpose nouns stay Inbox-only. |
