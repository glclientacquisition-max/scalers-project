...
| Filter tabs | `FilterTabs` + `filterTabClass`. Underline, `min-h-11`, active `border-[#0096FF] text-[#005CCC]`. Inbox List/Work and Today/Week share this. Contacts sort is `ContactSortSelect` (Last call · Name). Contacts segments All · Saved · Unsaved use `InboxFilterPills` (filled active pill, same DESIGN as Inbox purpose). Recents, Favourites, and Unsaved on Contacts are `ContactPileStrip` counts, not pills and not rail items. Inbox purpose piles (Needs you / Visits / Holds / …) stay Inbox-only nouns. |
