...
| Filter tabs | `FilterTabs` + `filterTabClass`. Underline, `min-h-11`, active `border-[#0096FF] text-[#005CCC]`. Inbox List/Work, Today/Week, and Contacts sort (Last call · Name) share this. Contacts segments All · Saved · Unsaved use `InboxFilterPills` (filled active pill, same DESIGN as Inbox purpose). Recents and Favourites on Contacts are `ContactPileCards` rate cards, not pills and not rail items. Inbox purpose piles (Needs you / Visits / Holds / …) stay Inbox-only nouns. |
