...
| Inbox overflow | More, long-press, right-click. One list: Select, Pin, then Mark done (return calls) / Archive, or Unarchive on Archived. Hairline between stay and leave. Desktop menu right-aligns to More (`placeInboxOverflowMenu`) so it does not cover the dock. Mute, Assign, Label, Delete, Mark unread, and Snooze stay off this list. Archive opens from a list row, not a filter chip. |
