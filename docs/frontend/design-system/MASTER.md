...
| Contact strip | `ContactStrip` on `/calls/[id]` header. Same family as the Contacts Phase 1 list row: `RowIdentity`, name or Unsaved / Name this caller, one `contactListSubline` (`deskPreviewClass`). Call + WhatsApp when a number exists (`deskHitClass`, `tel:` / `wa.me` opened only). No `callId` write-back. Tap identity opens `/contacts/[id]`. Same row at every width. Never Online, last seen, or presence. |
