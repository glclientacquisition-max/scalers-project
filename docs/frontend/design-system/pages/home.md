...
Do not mix requests/appointments into the Home queue until those lists share Calls language.

Empty: if no new leads, Calls empty-state language (`No new call leads`) plus the one derived next action in the aside.

Updates strip stays if `liveBulletinItems` has rows (already real).
