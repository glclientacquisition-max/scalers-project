...
**Ticket strip:** `/calls/[id]` header uses `ContactStrip` in this same family (avatar, one fact, opened-only Call + WhatsApp). List Phase 1 chrome stays on `/contacts`. Do not restyle the list to match the ticket, or the ticket to invent a second row language.
