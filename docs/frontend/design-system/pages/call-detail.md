...
**Layout:** Phone (`<md`) is a full call. `md+` is rail, Inbox list, call or empty pane. List width is `20rem` / `24rem` / `28rem` by breakpoint, then owner-resizable. The list is a parallel `@inbox` slot, not nested in the call page. Mail rows in that list column. One named scroll on the list, one on the call. Esc and Close return to Inbox. Filter tabs stay on the open call.
