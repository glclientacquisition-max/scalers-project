...
**Layout:** No desk shell lockup. Thread height uses `--desk-header-h: 0`. Phone ticket chat hides the bottom tab bar (`data-ticket-chat`; `--desk-tabbar-h: 0`). `DeskBack` is the escape. The `md+` rail stays. Fixed ticket header (`DeskBack` + person + More). `data-desk-bleed` fills the column next to `DeskRail` on `md+`. From `lg`, summary left and transcript right (`lg:grid-cols-[minmax(18rem,var(--ticket-summary-w,22rem))_1px_minmax(0,1fr)]`). Drag the gutter or use arrow keys. Floor 18rem summary, 20rem thread, cap 32rem. Stored in `localStorage["scalers-ticket-split"]`. Double-click resets. Below `lg`, the same two panes stack in that source order inside one scrolling region. The action dock, optional Confirm or hold Done banner, and SMS dock stay full width under the split (one docked action zone at every width).

**Header:** `ContactStrip` in the same family as the Contacts Phase 1 list row. Avatar, display name (or Unsaved / Name this caller), one factual subline (`contactListSubline`: last call, Unsaved, or phone). Tap identity opens `/contacts/[id]`. Call + WhatsApp sit in the strip when a number exists (`deskHitClass`, `tel:` / `wa.me` opened only, reach stamp `opened`). No `callId`. No `lead_status` or Needs you invent from the strip. No Meta activity theater. Ticket-dock WhatsApp still writes follow-up when the owner uses that dock. Icon-only hits (Back, Call, WhatsApp, More, Jump, Send) show `DeskHint` on hover and focus. More is a portaled menu aligned to the ⋮ (`placeInboxOverflowMenu`): Archive, or Unarchive when the call is archived. Not a sheet. Archive writes the Inbox Undo payload before `DeskBack` so the list toast survives the remount. Opening the ticket stamps `calls.inbox_read_at` (clears the list unread dot). Needs you stays until Confirm, Done, or Archive. The stamp is read-only `signalLabel` in the summary pane (Return call, Visit confirmed, Hold, Human asked, Missed, Answered, Live). No New / Followed Up / Done taxonomy. No Online, last seen, presence, or active now. Never delivered or sent without channel evidence.
...
**Thread:** Conversation. Jump to latest appears when scrolled away from the bottom. No FAQ ideas, Ask AI, or sparkle chrome.
...
**SMS dock:** Shown when `needsYou` and the call is not archived. Growing input plus Send. Send stays the filled primary. Uses `sendInboxReplySms`, distinct from auto-SMS on Confirm or Done. WhatsApp stays on the action dock. Spans the viewport under the split so Send stays docked at the bottom. No Polish wand.
