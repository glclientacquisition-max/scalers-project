...
**Header:** `ContactStrip` in the same family as the Contacts Phase 1 list row. Avatar, display name (or Unsaved / Name this caller), one factual subline (`contactListSubline`: last call, Unsaved, or phone). Tap identity opens `/contacts/[id]`. Call + WhatsApp sit in the strip when a number exists (`deskHitClass`, `tel:` / `wa.me` opened only, reach stamp `opened`). No `callId`. No `lead_status` or Needs you invent from the strip. No Meta activity theater. Ticket-dock WhatsApp still writes follow-up when the owner uses that dock. Icon-only hits (Back, Call, WhatsApp, More, Jump, Polish, Send) show `DeskHint` on hover and focus. More is a portaled menu aligned to the ⋮ (`placeInboxOverflowMenu`): Archive, or Unarchive when the call is archived. Not a sheet. Archive writes the Inbox Undo payload before `DeskBack` so the list toast survives the remount. Opening the ticket stamps `calls.inbox_read_at` (clears the list unread dot). Needs you stays until Confirm, Done, or Archive. The stamp is read-only `signalLabel` in the summary pane (Return call, Visit confirmed, Hold, Human asked, Missed, Answered, Live). No New / Followed Up / Done taxonomy. No Online, last seen, presence, or active now. Never delivered or sent without channel evidence.
