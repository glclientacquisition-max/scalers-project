...
function rateLimitScan(tenantId: string): string | null {
  ...
  if (prev.length >= max) {
    return "Scan rate limit. Wait a few minutes, or review the queue you already have.";
  }
  prev.push(now);
...
async function persistQueueFields(opts: {
  ...
  const { error } = await workspace.client
    ...
    .eq("id", opts.tenantId);
  if (error) {
    console.error("[settings.geminiScan]", error.message);
...
export async function geminiScanRecentCallsAction(
  ...
  if (batchSize > 10 && !confirmed) {
    return {
      ...
      batchSize,
      message: `Scan will listen to up to ${batchSize} recent recordings. Confirm to continue.`,
    };
  }
  ...
  if (!process.env.GEMINI_API_KEY) {
    return {
      error:
        "Scan is not available. Try again later.",
    };
  }
  ...
  if (persistErr) {
    console.error("[settings.geminiScan]", persistErr.message);
...
export async function queueGeminiCandidateForRecordingAction(
  ...
  return {
    ...
    message:
      "Queued for Practice. Record real audio.",
  };
}
