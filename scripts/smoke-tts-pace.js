 ...
 *
 * 1) Confirms default speedForLanguage('en') === 1.0 when env unset
 * 2) If SONIOX_API_KEY is set, synthesizes the same line at 0.95 and 1.0
...
async function main() {
  ...
  const envSpeed = process.env.SONIOX_TTS_SPEED || '(unset → code default 1.0)';
  console.log(`SONIOX_TTS_SPEED env: ${envSpeed}`);
  console.log(`speedForLanguage(en): ${effectiveEn}`);

  if (!process.env.SONIOX_TTS_SPEED && !process.env.VOICE_PROFILE && effectiveEn !== 1) {
    console.error('FAIL: expected default EN speed 1.0');
    process.exit(1);
  }
  ...
  // Expect ~0.95/1.0 ≈ 0.95 duration ratio; allow noise/jitter.
